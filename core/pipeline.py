...
from core.router import need_vision, RequestCache
...
from utils.logger import stats  # PR12：导入统计模块
...
class AppPipeline:
    ...
    def __init__(self) -> None:
        ...
        self._last_vision_result: str | None = None
    ...
    def process_audio(self, audio_path: Path) -> tuple[str, str, Path]:
        """完整的音频处理流程：ASR → 缓存检查 → 意图路由 → LLM/VLM → TTS"""
        try:
            # Step 1：ASR 识别
            ...
            if cached_reply:
                ...
                stats.record_request_cache_hit()
                return text, cached_reply, Path()
            ...
            if need_vision(text) and self._current_frame is not None:
                ...
                if motion_detect(self._prev_frame, self._current_frame):
                    ...
                    stats.record_vision_cache_hit()  # PR12：记录缓存命中
                ...
                if not reply:
                    ...
                    stats.record_llm_call()  # PR12：记录 LLM 调用
            else:
                # 调用 LLM
                messages = (
                    [{"role": "system", "content": SYSTEM_PROMPT}]
